...
class PredictorWorkerThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            s = 'FFFJkA, ' + str(len(batched)), len(futures), len(outputs)
    ...
    def run33(self):
        while True:
            ...
            ts = time.time() - 1481804971.021994
            outputs = self.func(batched)

            outputs.append([ts] * len(futures))
            print ('appending timestamp to outputs')
            #import pdb
            #pdb.set_trace()
...
class MultiThreadAsyncPredictor(AsyncPredictorBase):
    ...
    def put_task(self, dp, callback=None):
        ...
        # KURWA: LOGMAC
        logger.debug("Q-debug id=89jdslk: MultiThreadAsyncPredictor self.input_queue before put, size: {qsize} / {maxsize}".format(
                    qsize=self.input_queue.qsize(), maxsize=self.input_queue.maxsize))

        #print("Q-debug id=89jdslk: MultiThreadAsyncPredictor self.input_queue before put, size: ",
