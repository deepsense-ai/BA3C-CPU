...
class SimulatorProcessStateExchange(SimulatorProcessBase):
    ...
    def __init__(self, idx, pipe_c2s, pipe_s2c):
        ...
        print ('\n\n CREATING THE SIMULATOR PROCESS STATE EXCHANGE!!!')
    ...
    def run(self):
        ...
        while True:
            c2s_socket.send(dumps(
                (self.identity, state, reward, isOver, ts)),
                copy=False)
            #t.grel here we get the action
            (action, ts) = loads(s2c_socket.recv(copy=False).bytes)
            #print('action: ', action, 'ts: ', ts)
...
class SimulatorMaster(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            ident, state, reward, isOver, ts = msg
